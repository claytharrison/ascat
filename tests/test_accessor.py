...
class TestGridAccessor(unittest.TestCase):
    ...
    def test_coord(self):
        ...
        dates = (datetime(2021, 1, 17), datetime(2021, 1, 20))
        swath_ds = self.sgf.extract(dates[0], dates[1])#, bbox=bounds)
        self.assertTrue(swath_ds.gridds.coord.size > 0)
        self.assertTrue(swath_ds.gridds.coord == swath_ds["location_id"])
        print(swath_ds.gridds.coord)

    def test_sel_latlon(self):
        bounds = (45, 50, 10, 20) #latmin, latmax, lonmin, lonmax
        dates = (datetime(2021, 1, 17), datetime(2021, 1, 20))
        swath_ds = self.sgf.extract(dates[0], dates[1])#, bbox=bounds)
        self.assertTrue(swath_ds.gridds.sel_latlon(6.5, 165.2)["location_id"].load().size > 0)
        np.testing.assert_array_equal(swath_ds.gridds.sel_latlon(6.5, 165.2)["location_id"].load(),
                                      swath_ds.sel(location_id=741858)["location_id"].load())
